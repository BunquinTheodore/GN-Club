...
export function ServiceBento({ full = false }: { full?: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-8 md:auto-rows-[220px]">
        ...
        return (
          <motion.div
            ...
            className={`group relative overflow-hidden rounded-2xl border border-glass-border ${
              full ? "md:h-auto md:min-h-[280px]" : ""
            } ${spanClasses[service.span]}`}
          >
            <DuotoneImage src={getMedia(service.media)} alt={service.title}  />

            <div className="relative flex h-full flex-col justify-end p-6">
              <Icon className="mb-3 h-6 w-6 text-lime" strokeWidth={1.5} />
              <h3 className="font-display text-xl text-fog">{service.title}</h3>
              <p className="mt-1 text-sm text-fog-dim">{service.blurb}</p>

              <ul className={listClasses}>
                {service.items.map((item) => (
                  <li key={item} className="flex items-center gap-2 py-0.5 text-xs text-fog-dim">
                    <span className="h-1 w-1 rounded-full bg-gradient-to-r from-cyan to-amber" />
                    {item}
                  </li>
                ))}
              </ul>
